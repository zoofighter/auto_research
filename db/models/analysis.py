...
class AnalysisSession(Base):
    ...
    generated_questions: Mapped[Optional[str]] = mapped_column(Text)  # JSON 직렬화된 질문 리스트
    ...
    hitl_feedbacks: Mapped[list["HitlFeedback"]] = relationship(back_populates="session")  # noqa: F821
...
class WebSearchResult(Base):
    ...
    result_snippet: Mapped[Optional[str]] = mapped_column(Text)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
