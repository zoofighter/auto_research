...
class GeneratedReport(Base):
    ...
    report_md_path: Mapped[str | None] = mapped_column(String(500))
    report_pdf_path: Mapped[str | None] = mapped_column(String(500))
    ppt_path: Mapped[str | None] = mapped_column(String(500))
    quality_score: Mapped[float | None] = mapped_column(Float)      # LLM 자체 평가 0.0~1.0
...
class ReportSource(Base):
    ...
    source_id: Mapped[int | None] = mapped_column(Integer)          # 해당 테이블의 id
    excerpt: Mapped[str | None] = mapped_column(Text)               # 인용 구절
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
