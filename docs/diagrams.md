...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  [데이터 소스]              [저장소]              [멀티 에이전트]            [출력]
  
  PDF 리포트 ──┐                              ┌─ StockAgent ─ HITL-1,2,4
  DART 공시  ──┼──▶ CollectionAgent ──┬──▶ SQLite      │  (005930)    ├──┐
  뉴스 기사  ──┘                      └──▶ ChromaDB ──▶│                  │
                                                       ├─ StockAgent ─ HITL-1,2,4 │
                                                       │  (000660)    │   │
                                                       │                  │
                                                  SupervisorAgent         │
                                                  (Send() × N)    │   │
  ...
  │                 StockAgent 내부 흐름                   │
  │                                                      │
  │  analyze → question ⇄ HITL-1 → search               │
  │               ↑                    ↓                 │
  │           HITL-4               synthesize ⇄ HITL-2  │
  │               ↑                    ↓                 │
  │               └──── evaluate ◀─────┘                 │
  │               (quality < 0.7, 최대 3회)               │
  └──────────────────────────────────────────────────────┘
  ...
  │                  HITL 운영 모드                        │
  │                                                      │
  │  FULL-AUTO  : HITL 전체 스킵                          │
  │  SEMI-AUTO  : HITL-1, HITL-2만 활성 (기본값)           │
  │  FULL-REVIEW: 모든 HITL 활성, 타임아웃 없음             │
  │                                                      │
  │  알림 채널: Telegram Bot                              │
  └──────────────────────────────────────────────────────┘
```
